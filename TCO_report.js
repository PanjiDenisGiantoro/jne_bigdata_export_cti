...
function logErrorToFile(jobId, origin, destination, service, userId, errorMessage) {
...
reportQueue.process(async (job) => {
    return await Sentry.startSpan({name: 'Process Report TCO Job' + job.id, jobId: job.id}, async (span) => {
        ...
        try {
            ...
            await Sentry.startSpan({name: 'Log Error to File'+ job.id, jobId: job.id}, async () => {
                ...
                logErrorToFile(job.id, origin, destination, service, user_id, error.message);
...
reportQueueTCI.process(async (job) => {
...
reportQueueDCI.process(async (job) => {
    return await Sentry.startSpan({name: 'Process Report DCI Job' + job.id, jobId: job.id}, async (span) => {
...
reportQueueDCO.process(async (job) => {
    return await Sentry.startSpan({name: 'Process Report DCO Job' + job.id, jobId: job.id}, async (span) => {
        const {origin, destination, froms, thrus,service,  user_id, dateStr} = job.data;
        console.log('Processing job with data:', job.data);
...
async function estimateDataCount({ origin, destination, froms, thrus,service,  user_id }) {
        ...
        try {
            oracledb.getConnection(config, (err, conn) => {
                if (err) {
                    ...
                    if (destination !== '0') {
                        whereClause += ` AND SUBSTR(DESTINATION, 1, 8) LIKE :destination`;
                    ...
                    if (service !== '0') {
                        whereClause += ` AND SERVICES_CODE = :TM`;
...
async function estimateDataCountDCI({ origin, destination, froms, thrus, user_id, TM }) {
    return new Promise((resolve, reject) => {
        let connection;
        try {
            oracledb.getConnection(config, (err, conn) => {
...
async function fetchDataAndExportToExcel({ origin, destination, froms, thrus,service,  user_id, dateStr }) {
...
// Menambahkan alias ke setiap kolom
                worksheet.columns = [
                    ...
                    { header: 'COLLY', key: 'qty' },
                    { header: 'ZONA', key: 'zona_destination' },
                    { header: 'SERVICES CODE', key: 'services_code' },
                    { header: 'WEIGHT', key: 'weight' , style: { numFmt: '#,##0.00' }},
                    ...
                    { header: 'DELIVERY SPS', key: 'delivery_sps' , style: { numFmt: '#,##0.00' }},
                    { header: 'BIAYA TRANSIT', key: 'biaya_transit', style: { numFmt: '#,##0.00' } },
                    { header: 'BIAYA PENERUS', key: 'linehaul_first' , style: { numFmt: '#,##0.00' }},
                    { header: 'BIAYA PENERUS NEXT', key: 'linehaul_next', style: { numFmt: '#,##0.00' } },
...
async function fetchDataAndExportToExcelTCI({ origin, destination, froms, thrus, user_id, TM,user_session, dateStr }) {
    return new Promise(async (resolve, reject) => {
        let connection;
        try {
            connection = await oracledb.getConnection(config);
...
async function fetchDataAndExportToExcelDCI({ origin, destination, froms, thrus, user_id, TM, dateStr }) {
    return new Promise(async (resolve, reject) => {
        let connection;
        try {
            connection = await oracledb.getConnection(config);
            ...
            for (let i = 0; i < chunks.length; i++) {
                ...
                worksheet.columns = [
                    { header: 'NO', key: 'NO' },
                    { header: 'CNOTE NO', key: 'CONNOTE_NUMBER' },
                    { header: 'CNOTE DATE', key: 'CONNOTE_DATE' },
                    { header: 'ORIGIN', key: 'ORIGIN' },
                    { header: 'DESTINATION', key: 'DESTINATION' },
                    { header: 'ZONA DESTINATION', key: 'ZONA_DESTINATION' },
                    { header: 'SERVICE CONNOTE', key: 'SERVICE_CONNOTE' },
                    ...
                    { header: 'WEIGHT BAG', key: 'WEIGHT_BAG' },
                    { header: 'PRORATED WEIGHT', key: 'PRORATED_WEIGHT' },
                    { header: 'TRANSIT FEE', key: 'TRANSIT_FEE' },
                    { header: 'HANDLING FEE', key: 'HANDLING_FEE' },
                    { header: 'OTHER FEE', key: 'OTHER_FEE' },
                    { header: 'DOWNLOAD DATE', key: 'DOWNLOAD_DATE' }
...
app.get('/getreporttco', async (req, res) => {
    try {
        ...
        const estimatedDataCount = await estimateDataCount({ origin, destination, froms, thrus, service,user_id });
...
app.get('/getreportdci', async (req, res) => {
    try {
        ...
        // Estimasi jumlah data
        const estimatedDataCount = await estimateDataCountDCI({ origin, destination, froms, thrus, user_id, TM });
...
app.get('/getreportdco', async (req, res) => {
    try {
        ...
        // Estimasi jumlah data
        const estimatedDataCount = await estimateDataCountDCO({ origin, destination, froms, thrus, service,user_id });
