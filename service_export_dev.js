...
// Menambahkan alias ke setiap kolom
            ...
            const zipFileName = path.join(__dirname, 'file_download_dev', `TCOReport_${user_id}_${dateStr}_${timeStr}.zip`);
...
--                      AWB_DATE AS CONNOTE_DATE,
            ...
            const zipFileName = path.join(__dirname, 'file_download_dev', `TCIReport_${user_id}_${dateStr}_${timeStr}.zip`);
            const output = fs.createWriteStream(zipFileName);
            const archive = archiver('zip', {
                zlib: {level: 5}
            });
...
--                     CASE WHEN SERVICES_CODE LIKE 'JTR%' THEN 'LOG' ELSE 'EXP' END TIPE, 
            ...
            const zipFileName = path.join(__dirname, 'file_download_dev', `DCIReport_${user_id}_${dateStr}_${timeStr}.zip`);
            const output = fs.createWriteStream(zipFileName);
            const archive = archiver('zip', {
                zlib: {level: 5}
            });
...
--                         END AS TIPE,
            ...
            const zipFileName = path.join(__dirname, 'file_download_dev', `DCOReport_${user_id}_${dateStr}_${timeStr}.zip`);
            const output = fs.createWriteStream(zipFileName);
            const archive = archiver('zip', {
                zlib: {level: 5}
            });
...
app.use('/file_download_dev', express.static(path.join(__dirname, 'file_download_dev')));
